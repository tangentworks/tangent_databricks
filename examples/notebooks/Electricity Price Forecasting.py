...
# MAGIC %md
# MAGIC # Tangent Databricks Example - Electricity Price Forecasting
...
file_path = '/Workspace'+dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get().rsplit('/', 2)[0]+'/data/day_ahead_price.csv'
...
visualization.data(df=tangent_dataframe,timestamp=timestamp_column,target=target_column,predictors=predictors[:5])
...
training_to = str(tangent_dataframe[timestamp_column].quantile(9/10))
