...
# MAGIC %md
# MAGIC # Tangent Databricks Example - Gas Consumption Forecasting
...
file_path = '/Workspace'+dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get().rsplit('/', 2)[0]+'/data/gas_consumption.csv'
tangent_dataframe = pd.read_csv(file_path)
group_keys = []
timestamp_column = "Date"
target_column = "GasCons"
...
auto_forecasting_configuration = {
    ...
    'engine': {
        ...
        'prediction_to': {'base_unit': 'day','value': 7},
        # 'target_offsets': 'none',
...
result_table_df = tw.PostProcessing().result_table(forecasting=tangent_auto_forecast).sort_values(by=['date_from','samples_ahead'])
...
visualization.predictions(result_table_df[result_table_df.groupby('timestamp').cumcount()==0].copy())
