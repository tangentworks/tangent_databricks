...
# MAGIC %md
# MAGIC # Tangent Databricks Example - Forecasting
...
class user_defined:
    def build_model(data,configuration):
        time_series = tw.TimeSeries(data=data)
        time_series.validate()
        tangent_forecast = tw.Forecasting(time_series=time_series,configuration=configuration)
        tangent_forecast.build_model()
        return tangent_forecast.model.to_dict()

    def predict(data,configuration,model):
        timestamp = [f for f in data.columns if f not in [f['name'] for f in model['model_zoo']['variable_properties']]][0]
        min_offset = min([f['data_from'] for f in model['model_zoo']['variable_properties']])
        target_name = model['model_zoo']['target_name']
        last_target_index = data[target_name].last_valid_index()
        last_target_timestamp = data[timestamp].iloc[last_target_index]
        predict_dataframe = data.iloc[last_target_index+min_offset:]
        time_series = tw.TimeSeries(data=predict_dataframe)
        time_series.validate()
        tangent_forecast = tw.Forecasting().from_model(model=model) 
        predictions = tangent_forecast.forecast(configuration=configuration,time_series=time_series)
        return predictions[predictions['timestamp']>last_target_timestamp].copy()
...
for tangent_job in build_model_tangent_jobs:
    ...
    spark_jobs.append((tangent_job['id'],user_defined.build_model,{'data':data,'configuration':build_model_configuration}))
...
for tangent_job in predict_tangent_jobs:
    ...
    spark_jobs.append((tangent_job['id'],user_defined.predict,{'data':data,'configuration':predict_configuration,'model':model}))
...
for tangent_job in build_model_tangent_jobs:
    job_id = tangent_job['id']
    model = [f['result'] for f in tw_parallel_model_building if f['id']==job_id][0]
