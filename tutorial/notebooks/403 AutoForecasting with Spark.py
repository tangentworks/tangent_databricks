...
csv_df = csv_df[(csv_df['center_id']!=10)&(csv_df['meal_id']!=1109)]
...
# MAGIC %md
# MAGIC Here we bring together the general configuration and the individual time series that are filtered from the large dataset using the parameters.  
# MAGIC Then the auto_forecast functions are sent to Spark and the jobs are executed in parallel.
...
for tangent_job in tangent_jobs:
    ...
    spark_jobs.append((tangent_job['id'],tw.forecasting.auto_forecast,{'configuration':auto_forecasting_configuration,'dataset':dataset}))
