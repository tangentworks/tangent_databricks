...
class visualization:
    ...
    def predictor_importance(df):
        ...
        fig = go.Figure(go.Bar(x=v_data[x_axis], y=v_data[y_axis],text=round(v_data[y_axis],2),textposition='auto'))
        fig.update_layout(height=500,width=1000,title_text='Predictor Importances',xaxis_title=x_axis,yaxis_title=y_axis)
    ...
    def all_properties(properties):
        fig = px.bar(properties[properties['importance']>0], x='target_column', y="rel_importance", color="name", barmode = 'stack',hover_data=['id'])
...
# MAGIC
# MAGIC In the Tangent Python Databricks package, there exists the SparkParallelProcessing class which allows the user to send jobs to Tangent in parallel. The detection process can be parallelized in different ways. In this example we show how to run in parallel each step in the detection process seperately and across all the combinations.  
# MAGIC
...
for tangent_job in tangent_jobs:
  ...
  spark_jobs.append((tangent_job['id'],tw.anomaly_detection.build_model,{'configuration':configuration,'dataset':tangent_dataframe}))
...
for tw_anomaly_detection_build_model in tw_anomaly_detection_build_models:
    model = tw_anomaly_detection_build_model['result']
    spark_jobs.append((tw_anomaly_detection_build_model['id'],tw.anomaly_detection.detect,{'dataset':tangent_dataframe,'model':model}))
...
visualization.all_properties(properties=tangent_properties_df)
